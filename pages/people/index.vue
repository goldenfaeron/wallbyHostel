<template>
	<div>
		<v-img
			:src="require('@/assets/humaans/humaaans.png')"
		...
		<Title>
			<h1 class="text-center primary--text">People in {{$store.state.city}}</h1>
			<h2
				class="secondary--text"
				style="text-align: center;"
			>{{response.length}} friendly locals in {{$store.state.city}}</h2>
		</Title>

		<v-container grid-list-lg>
			<v-layout row wrap>
				<v-flex xs12 sm6 md4 v-for="(item, index) in response" :key="index">
					<CardPerson :props="item"></CardPerson>
				</v-flex>
			</v-layout>
		</v-container>
	</div>
